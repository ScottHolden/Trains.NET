...
namespace Trains.NET.Comet
{
    public class MiniMapDelegate : AbstractControlDelegate, IDisposable
    {
        ...
        public override void Draw(SKCanvas canvas, RectangleF dirtyRect)
        {
            ...
            (int col, int row) = _pixelMapper.ViewPortPixelsToCoords(0, 0);
            (int width, int height) = _pixelMapper.ViewPortPixelsToCoords(_pixelMapper.ViewPortWidth, _pixelMapper.ViewPortHeight);

            canvas.DrawRect(col, row, width - col, height - row, _viewPortPaint);
